...
NrMacSchedulerOfdma::NrMacSchedulerOfdma()
    : NrMacSchedulerTdma()
{
    std::cout << "--> NrMacSchedulerOfdma" << std::endl;
}
...
NrMacSchedulerOfdma::AssignULRBG(uint32_t symAvail, const ActiveUeMap& activeUl) const
{
    ...
    for (const auto& el : activeUl)
    {
        ...
        std::string sched = fl::GlobalMetrics::Instance().GetSchedulerCode();
        if (sched == "FL") {            
            
            {
                GetFirst GetUe;                
                const size_t initialFl = std::count_if(ueVector.begin(), ueVector.end(),
                    [&](const UePtrAndBufferReq& it){
                        auto u = GetUe(it);
                        return (u->m_ueExtInfo && u->m_ueExtInfo->GetIsFL());
                    });

                const bool hasFlWithData = std::any_of(ueVector.begin(), ueVector.end(),
                    [&](const UePtrAndBufferReq& it){
                        auto u = GetUe(it);
                        const bool isFl   = (u->m_ueExtInfo && u->m_ueExtInfo->GetIsFL());
                        const bool hasBsr = (it.second > 0); 
                        return isFl && hasBsr;
                    });
                ...
                {                    
                    ...
                    if (initialFl > 0)
                    {
                        const size_t totalNonFl = ueVector.size() - initialFl;                       
                        const double frac = std::max(0.0,
                            1.0 - static_cast<double>(initialFl) / static_cast<double>(Fmax));
                       
                        const double raw = static_cast<double>(totalNonFl) * std::pow(frac, alpha);
                        size_t quota = static_cast<size_t>(std::ceil(std::max(0.0, raw)));                       
                        quota = std::min(quota, totalNonFl);
                    ...
                    size_t keptNonFl = 0;
                    ueVector.erase(
                        std::remove_if(ueVector.begin(), ueVector.end(),
                            [&](const UePtrAndBufferReq& it){
                                auto u = GetUe(it);

                                const bool isFl = (u->m_ueExtInfo && u->m_ueExtInfo->GetIsFL());
                                const bool hasUlHarqPending = false; 
                                
                                if (isFl || hasUlHarqPending) {
                                    return false;
                                }
                                
                                if (keptNonFl < maxNonFl) {
                                    ++keptNonFl;
                                    return false;
                                }
        ...
        while (resources > 0)
        {
            ...
            if (GetUe(*schedInfoIt)->m_ueExtInfo->GetIsFL()) { 
            	fl::GlobalMetrics::Instance().AddUlRbgUeFL(1);              	
            }
            else {
            	fl::GlobalMetrics::Instance().AddUlRbgUeNFL(1);             	
            }       
