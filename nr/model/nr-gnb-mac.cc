    ...
    for (auto& varTtiAllocInfo : ind.m_slotAllocInfo.m_varTtiAllocInfo)
    {
                 ...
                 varTtiAllocInfo.m_dci->m_format == DciInfoElementTdma::UL)
        {
            ...
            if (Simulator::Now() > Seconds(1.0)) {             	
            	if (fl::GlobalMetrics::Instance().HasRntiUE(dciElem->m_rnti)) {
            	    fl::GlobalMetrics::Instance().IncSchedCfgCntUeFL();
            	    fl::GlobalMetrics::Instance().AddUlMcsUeFL((int)dciElem->m_mcs);            		 	
            	    fl::GlobalMetrics::Instance().AddUlNumSymUeFL((int)dciElem->m_numSym);
            	    fl::GlobalMetrics::Instance().AddUlTbSizeUeFL((int)dciElem->m_tbSize);
            	    fl::GlobalMetrics::Instance().AddUlNdiUeFL((int)dciElem->m_ndi);
            	}
            }
        }
    }
